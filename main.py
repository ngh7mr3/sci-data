...
if len(sys.argv) < 3:
    sys.exit(
        "You should include start date in format DD/MM/YYYY and experiment duration!")

START_DATE = sys.argv[1]
reg_exp = r'(0[1-9]|[12][0-9]|3[01])[- /.](0[1-9]|1[012])[- /.](19|20)\d\d'
if not re.match(reg_exp, START_DATE):
...
class Parser():
    ...
    def __init__(self, name, template_url, time_offset,
                 start_line, hour_col, minute_col, collumns_to_save):
        self.name = name
    ...
    def calc_dates(self):
        ...
        start_date += dt.timedelta(hours=self.time_offset)
            ...
            if not os.path.isfile(file_name.replace('zip', 'txt')):
                try:
                    ...
                    if file_format == 'zip':
                        ...
                        file_name = zip_file.infolist()[0].filename
                        zip_file.extractall()
                        zip_file.close()
                        os.system(
                            ('del ' if os.name == 'nt' else 'rm ') +
                            zip_file_name)
                    elif file_format != 'txt':
                        sys.exit("Unsupported file has been downloaded!")

                    self.files_saved.append(file_name)

                except Exception:
                    print(
                        "Warning!\nNo such file or a directory provided by",
                        base_url)
            else:
        ...
        for ind, file in enumerate(self.files_saved):
            if ind == 0:
                ...
                self.collect_file_data(
                    file, end=24 if self.start_hour == 0 else self.start_hour)
            else:
    ...
    def collect_file_data(self, file, start=0, end=24, mode='hours'):
        ...
        data_lines = [
            i.split() for i in data_file.readlines()[
                self.start_line - 1:]]
        data_file.close()
        ...
        for line in data_lines:
            ...
            if cur_hour > next_hour and cur_hour < end:
                ...
                for k in range(next_hour,
                               cur_hour if cur_minute == 0 else cur_hour + 1):
                    print("\nWARNING!")
                    print(
                        "MISSING HOURLY DATA IN FILE " +
                        file +
                        " FOR HOUR " +
                        str(k))
                    print("THE DATA WILL BE NULLIFIED FOR THIS HOUR")
                    self.parsed_data.append(
                        ['0' for i in range(len(self.collumns_to_save))])
    ...
    def get_time(self, line):
        if self.hour_col != self.minute_col:
            return int(line[self.hour_col - 1]),\
                int(line[self.minute_col - 1])
        else:
            # Parsing HHMM template
            return int(line[self.hour_col - 1][:2]),\
                int(line[self.hour_col - 1][2:])

    def clear_downloads(self):
        for file in self.files_saved:
            os.system(('del ' if os.name == 'nt' else 'rm ') + file)
...
if __name__ == "__main__":
    ...
    MSC_mag = Parser("MSC_mag", url_msc_mag, 0, 5, 4, 5, [7, 8, 9])
